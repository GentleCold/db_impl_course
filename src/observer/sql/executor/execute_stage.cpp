...
void orderby_exec(const Selects &selects, TupleSet *res_tuples) {
  if (selects.order_num != 0) {
    std::vector<Tuple> temp;
    for (auto &tuple : res_tuples->tuples()) {
      temp.push_back(std::move(tuple));
    }

    std::sort(temp.begin(), temp.end(), [&](const auto &t1, const auto &t2) {
      for (int i = 0; i < selects.order_num; ++i) {
        OrderDescription desc = selects.order_des[i];
        int order_index = 0;
        ...
        int cmp = t1.get(order_index).compare(t2.get(order_index));
        if (cmp == 0) {
          continue;
        }

        if (desc.type == kOrderAsc) {
          return cmp < 0;
        } else {
          return cmp > 0;
        }
    ...
    auto schema = res_tuples->get_schema();
    res_tuples->clear();
    res_tuples->set_schema(schema);
    for (auto &tuple : temp) {
      res_tuples->add(std::move(tuple));
    }
  }
}
